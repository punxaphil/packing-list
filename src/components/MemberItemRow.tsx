...
export function MemberItemRow({ memberItem, item }: { memberItem: MemberItem, item: Item }) {
  ...
  return <div key={memberItem.id} className="ml-5">
    <Checkbox checked={memberItem.checked} onClick={() => toggleMember(memberItem.id)} />
    <Span strike={memberItem.checked} className="ml-1">{getName(members, memberItem.id)}</Span>
  </div>;
}
